...
std::string safeSubstring(const std::string& str, size_t maxBytes) {
...
auto generatesummary(const std::string& post_content) -> std::string {
    ...
    std::string valid_content = ensureValidUTF8(post_content);
    ...
    if (res && res->status == 200) {
        try {
            ...
            if (res_json.contains("choices") && !res_json["choices"].empty()) {
                ...
                return "[Error] Unexpected response format: " + res->body;
            }
        ...
        return "[Error] API Request failed: " + error_msg + " Details: " + error_details;
    }
}
...
auto main(int argc, char *argv[]) -> int {
    ...
    for(const auto& post_dire : std::filesystem::directory_iterator(POST_ROOT)) {
        ...
        if (full_content.length() > 10000) {
            full_content = safeSubstring(full_content, 10000);
        }
